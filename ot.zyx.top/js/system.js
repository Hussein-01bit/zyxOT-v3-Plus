...
window.this_long_time = y1 + "-" + ((m1 < 10) ? '0' + m1 : m1) + "-" + ((dys1 < 10) ? '0' + dys1: dys1) + " " + ((h1 < 10) ? '0' + h1 : h1) + ":" + ((min1 < 10) ? '0' + min1 : min1) + ":" + ((s1 < 10) ? '0' + s1 : s1);

$('#web-time').text('服务器时间：' + this_long_time);
...
function getUserLink(username, realname, color) {
	...
	if (localStorage.getItem('zyxOT-v3-user=' + username + '?password') == -1) {
		return '/';
	}
	if (realname) {
	...
	return '<a class="' + className + '" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/user.html?' + username + '">' + text + '</a>';
}
function getUserSpan(username, realname, color) {
	...
	if (realname) {
		text = text + ' <span class="d-inline-block">(' + realname + ')</span>';
	}
	if (color) {
		className += ' uoj-username-' + color;
	}
...
function replaceWithHighlightUsername() {
	...
	var realname = localStorage.getItem('zyxOT-v3-user=' + username + '?tag');
	var color = localStorage.getItem('zyxOT-v3-user=' + username + '?color');
	if (username.charAt(0) == '@') {
		color = localStorage.getItem('zyxOT-v3-user=' + username.substr(1) + '?color');
	}
	if (username == "加密系统官方" || username == "@加密系统官方") {
		color = "red";
		$(this).replaceWith(getUserSpan(username, realname, color));
		return;
	}
	if (localStorage.getItem('zyxOT-v3-user=' + username + '?banned') == 1) {
		if (localStorage.getItem('zyxOT-v3-user=' + localStorage.getItem("zyxOT-v3-username") + '?manage') == null) 
			username = "封禁用户", realname = null;
		else 
			realname = "banned";
	}
	if (username.charAt(0) == '@') {
		if (localStorage.getItem('zyxOT-v3-user=' + username.substr(1) + '?banned') == 1) {
			if (localStorage.getItem('zyxOT-v3-user=' + localStorage.getItem("zyxOT-v3-username") + '?manage') == null) 
				username = "@封禁用户", realname = null;
			else 
				realname = "banned";
...
function uoj_click_zan_up(id, type) {
	var username = localStorage.getItem("zyxOT-v3-username");
	var cnt = localStorage.getItem("zyxOT-v3-click_zan-type=" + type + "&id=" + id);
	var val;
	if (cnt == null) 
		cnt = 0;
	if (username != null) {
		val = localStorage.getItem("zyxOT-v3-click_zan-type=" + type + "&id=" + id + "&user=" + username);
		if (val == null) 
			val = 0;
	}
	else {
	...
	localStorage.setItem("zyxOT-v3-click_zan-type=" + type + "&id=" + id, cnt);
	localStorage.setItem("zyxOT-v3-click_zan-type=" + type + "&id=" + id + "&user=" + username, val);
	location.href = "";
}
	...
	return this.each(function() {
		...
		var up_node = $('<a href="#" class="uoj-click-zan-up"><span class="glyphicon glyphicon-thumbs-up"></span>'+uojLocale('click-zan::good')+'</a>').click(function(e) {
			...
			if (username != null) 
				...
				$(this).html('<div class="text-danger">please <a href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/login.html">log in</a></div>');
		});
		...
		if (cnt_zan_see <= zan_cnt) {
			if (localStorage.getItem('zyxOT-v3-toushi?id=' + id + '&locker') != 1) {
				$(this).append(up_node);
				$(this).append(down_node);
			}
			else 
				$(this).removeClass('uoj-click-zan-block');
...
// top-dock
var top_dock = document.querySelector("#navbarSupportedContent-dock");
if (top_dock) {
	var top_dock_text = "";
	if (localStorage.getItem("zyxOT-v3-username") == null) {
		if (localStorage.getItem("zyxOT-v3-guest-translation") == "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/main.html">\n						<i class="bi bi-card-list"></i>\n						加密					</a>\n				</li>'
		if (localStorage.getItem("zyxOT-v3-guest-toushi") == "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/submit.html">\n						<i class="bi bi-pencil-square"></i>\n						投食					</a>\n				</li>'
		if (localStorage.getItem("zyxOT-v3-guest-help") == "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/faq.html">\n						<i class="bi bi-question-circle"></i>\n						帮助					</a>\n				</li>';
		if (localStorage.getItem("zyxOT-v3-guest-see_domain") == "true") {
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/domain.html">\n						<i class="bi bi-pie-chart"></i>\n						全系统管理					</a>\n				</li>';
		}
		if (localStorage.getItem("zyxOT-v3-guest-manage") == "true") {
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/manage.html">\n						<i class="bi bi-grid-3x3-gap"></i>\n						控制面板					</a>\n				</li>';
		}
	}
	else {
		if (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") == 1 || localStorage.getItem("zyxOT-v3-default-translation") == "true" && localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?unseed") != "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/main.html">\n						<i class="bi bi-card-list"></i>\n						加密					</a>\n				</li>'
		if (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") == 1 || localStorage.getItem("zyxOT-v3-default-toushi") == "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/submit.html">\n						<i class="bi bi-pencil-square"></i>\n						投食					</a>\n				</li>'
		if (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") == 1 || localStorage.getItem("zyxOT-v3-default-help") == "true" && localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?unhelped") != "true") 
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/faq.html">\n						<i class="bi bi-question-circle"></i>\n						帮助					</a>\n				</li>';
		if (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") == 1 || localStorage.getItem("zyxOT-v3-default-see_domain") == "true") {
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/domain.html">\n						<i class="bi bi-pie-chart"></i>\n						全系统管理					</a>\n				</li>';
		}
		if (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") == 1 || localStorage.getItem("zyxOT-v3-default-manage") == "true") {
			top_dock_text += '<li class="nav-item">\n					<a class="nav-link" href="' + localStorage.getItem("zyxOT-v3-deploy") + 'ot.zyx.top/manage.html">\n						<i class="bi bi-grid-3x3-gap"></i>\n						控制面板					</a>\n				</li>';
		}
	}
...
linkcss.href = localStorage.getItem("zyxOT-v3-deploy") + "ot.zyx.top/css/system.css";
linkcss.media = "all";
document.getElementsByTagName("head")[0].appendChild(linkcss);
// var linkjs = document.createElement("script");
// linkjs.type = "text/javascript";
// linkjs.src = localStorage.getItem("zyxOT-v3-deploy") + "ot.zyx.top/js/DB.js";
// document.getElementsByTagName("head")[0].appendChild(linkjs);

if (localStorage.getItem("zyxOT-v3-username") == null && localStorage.getItem("zyxOT-v3-guest-banned") == "false") {
...
else if (localStorage.getItem("zyxOT-v3-username") != null && localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?manage") != 1 && localStorage.getItem("zyxOT-v3-default-banned") == "false") {
	document.querySelector("title").innerHTML = "403 - 在线加密系统";
	document.querySelector(".uoj-content").innerHTML = '<div class="text-center"><div style="font-size:233px">403</div><p>禁止入内！ T_T</p></div>			';
}
else if (localStorage.getItem("zyxOT-v3-username") != null && localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?banned") == 1) {
	DB.query("update ({type : \"logout\", user : \"" + localStorage.getItem("zyxOT-v3-username") + "\", error : \"This user does banned.\"})");
}
else if (localStorage.getItem("zyxOT-v3-username") != null && (localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?password") == -1 || localStorage.getItem("zyxOT-v3-user=" + localStorage.getItem("zyxOT-v3-username") + "?password") == null)) {
	DB.query("update ({type : \"logout\", user : \"" + localStorage.getItem("zyxOT-v3-username") + "\", error : \"Not have this user.\"})");
}
...
// document.querySelector(".uoj-username, .uoj-score").style.fontFamily = '"Open Sans", "Open Sans", Seravek, "Segoe UI", Verdana, "PingFang SC", "Hiragino Sans GB", "Lantinghei SC", "Microsoft Yahei", "WenQuanYi Micro Hei", "sans"';
...
function remove_user() {
	...
	if (!confirm("确定注销账号吗？该操作不可逆！")) 
		return false;
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?banned");
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?color");
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?main-1");
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?main-2");
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?manage");
	localStorage.removeItem("zyxOT-v3-user=" + uoj_username + "?muted");
	localStorage.setItem("zyxOT-v3-user=" + uoj_username + "?password", -1);
	DB.query("update ({type : \"logout\", user : \"" + localStorage.getItem("zyxOT-v3-username") + "\", error : \"Not have this user.\"})");
	location.href = localStorage.getItem("zyxOT-v3-deploy") + "ot.zyx.top.html";
	return true;
}

document.querySelector('body').querySelectorAll('script').forEach(script => script.remove());;


if (DB.query("query ({type : \"checkuser\", user : \"" + localStorage.getItem("zyxOT-v3-username") + "\"})") == false) {
	DB.query("update ({type : \"logout\", user : \"" + localStorage.getItem("zyxOT-v3-username") + "\", error : \"This user can not have this pass.\"})");
}
